...
void ActorPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
	...
	if (!rclcpp::is_initialized())
	{
		...
		//rclcpp::init(argc, argv, "gazebo_client", rclcpp::init_options::NoSigintHandler);
		rclcpp::init(argc, argv, "gazebo_client", rclcpp::init_options::NoSigintHandler);
	}

	this->rosNode.reset(new rclcpp::Node::make_shared("gazebo_client"));
	...
	this->SetPoseService =
		this->rosNode->create_service<actor_services::srv::set_pose>("/" + this->actor->GetName() + "/SetActorPosition",
																	 &ActorPlugin::SetPoseCallback);
	...
	this->SetTargetService =
		this->rosNode->create_service<actor_services::srv::set_target>("/" + this->actor->GetName() + "/SetActorTarget",
																	   &ActorPlugin::SetTargetCallback);

	/*this->GetVelService = this->rosNode->advertiseService("/" + this->actor->GetName() + "/GetActorVelocity",
														  &ActorPlugin::GetVelCallback, this);*/

	this->GetVelService =
		this->rosNode->create_service<actor_services::srv::get_vel>("/" + this->actor->GetName() + "/GetActorVelocity",
																	&ActorPlugin::GetVelCallback);

	/*this->VelPublisher = this->rosNode->advertise<geometry_msgs::Twist>("/" + this->actor->GetName() + "/actor_vel", 1);*/

	rclcpp::Publisher<geometry_msgs::Twist>::SharedPtr VelPublisher =
		this->create_publisher<geometry_msgs::Twist>("/" + this->actor->GetName() + "/actor_vel", 1);
...
{
	rclcpp::ServiceClient GetVelClient = this->rosNode->serviceClient<actor_services::srv::get_vel>("/" + actor_name_ + "/GetActorVelocity");
	actor_services::srv::get_vel getvel_srv;
...
void ActorPlugin::OnUpdate(const common::UpdateInfo &_info)
{
	...
	//br.sendTransform(tf::StampedTransform(tf_transform, rclcpp::Time::now(), "default_world", this->actor->GetName()));
	//rclcpp::spinOnce();
...
bool ActorPlugin::GetVelCallback(actor_services::srv::get_vel::Request &req, actor_services::srv::get_vel::Response &res)
{
...
void ActorPlugin::QueueThread()
{
	...
	{
		this->rosQueue.callAvailable(rclcpp::WallDuration(timeout));
	}
}
...
void ActorPlugin::get_ros_parameters(const rclcpp::NodeHandlePtr rosNodeConstPtr)
{
